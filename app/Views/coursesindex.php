      ...
      <table class="min-w-full border border-gray-200 divide-y divide-gray-200">
        ...
        <tbody class="text-gray-800 text-sm">
          ...
          <tr class="<?= $index % 2 === 0 ? 'bg-white' : 'bg-gray-50' ?>">
            ...
            <td class="px-4 py-2"><?= esc($course['code']) ?></td>
            <td class="px-4 py-2"><?= esc($course['title']) ?></td>
            <td class="px-4 py-2"><?= esc($course['sks']) ?></td>
            <td class="px-4 py-2">
              <div class="flex space-x-2">
                <a href="/courses/edit/<?= $course['id'] ?>" class="bg-green-200 text-green-800 px-3 py-1 rounded hover:bg-green-300">
                  Edit
                </a>
                <form action="/courses/delete/<?= $course['id'] ?>" method="post" onsubmit="return confirm('Are you sure?')">
